...
import { createEditOps, createWriteOps, execCapture, extractCommandName } from "../src/ops";
...
describe("createWriteOps.writeFile", () => {
	...
	it("rejects write to read-only mount", async () => {
		...
		await expect(ops.writeFile("/skills/my-skill/SKILL.md", "data")).rejects.toThrow("refusing to write");
	});
});
...
describe("createWriteOps.mkdir", () => {
	it("rejects mkdir in read-only mount", async () => {
		...
		await expect(ops.mkdir("/skills/my-skill/subdir")).rejects.toThrow("refusing to mkdir");
	});
});
...
describe("createEditOps.writeFile", () => {
	it("rejects write to read-only mount with createWriteOps error message", async () => {
		...
		await expect(ops.writeFile("/skills/my-skill/SKILL.md", "content")).rejects.toThrow("refusing to write");
	});
});
