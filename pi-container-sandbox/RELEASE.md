...
发布通过 GitHub Actions 自动完成，触发条件是创建 GitHub Release（Release tag 格式 `pi-container-sandbox-v*`）。
...
gh release create pi-container-sandbox-v<新版本号> --target $COMMIT \
  --title "pi-container-sandbox v<新版本号>" --notes ""
```
