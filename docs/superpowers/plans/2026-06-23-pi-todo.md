...
**Tech Stack:** TypeScript (strict), typebox for schemas, `StringEnum` from `@earendil-works/pi-ai`, vitest, biome. Node `crypto.randomUUID()` for IDs.
...
- Dev dependencies: `@earendil-works/pi-tui`, `typebox`; add `@earendil-works/pi-ai` for `StringEnum`
...
- Schema enum uses `StringEnum` (not `Type.Union`/`Type.Literal`) for Google API compatibility
...
export default function (pi: ExtensionAPI) {
	...
	const TodoParams = Type.Object({
		action: StringEnum(["set", "update", "list"] as const),
		items: Type.Optional(
			Type.Array(
				Type.Object({
					id: Type.String(),
