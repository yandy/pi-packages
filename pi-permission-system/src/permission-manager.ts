...
export class PermissionManager implements ScopedPermissionManager {
	...
	check(intent: ResolvedAccessIntent, sessionRules?: Ruleset): PermissionCheckResult {
		...
		if (intent.kind === "path-values") {
			...
			return buildCheckResult(intent.surface, lookupValues, {}, intent.surface, intent.surface, fullRules);
		}
		...
		return buildCheckResult(surface, values, resultExtras, toolName, intent.surface, fullRules);
	}
}
