...
import { getActiveAgentName, getActiveAgentNameFromSystemPrompt, type SessionEntryView } from "../active-agent";
...
function getContextSystemPrompt(ctx: ForwarderContext): string | undefined {
	...
	try {
		...
		logPermissionForwardingWarning(null, "Failed to read context system prompt for forwarded permission metadata", error);
		return undefined;
	}
}
...
export class PermissionForwarder implements ApprovalRequester, InboxProcessor {
	...
	async processInbox(ctx: ForwarderContext): Promise<void> {
		...
		for (const fileName of requestFiles) {
			...
			await this.processSingleForwardedRequest(ctx, request, location, requestPath, currentSessionId);
		}
	...
	private async waitForForwardedApproval(
		...
		const targetSessionId = resolvePermissionForwardingTargetSessionId({
			hasUI: ctx.hasUI,
			isSubagent: isSubagentExecutionContext(ctx, this.subagentSessionsDir, this.registry),
		...
		const request = this.buildForwardedRequest(ctx, message, requesterSessionId, targetSessionId, forwarded);
		...
		try {
			...
			logPermissionForwardingError(this.logger, `Failed to write forwarded permission request '${requestPath}'`, error);
			return { approved: false, state: "denied" };
		}
	...
	private async pollForForwardedResponse(
		...
		logPermissionForwardingWarning(this.logger, `Timed out waiting for forwarded permission response '${responsePath}'`);
	...
	private async processSingleForwardedRequest(
		...
		if (shouldAutoApprovePermissionState("ask", this.config.current())) {
			this.logger.review("forwarded_permission.auto_approved", forwardedPermissionLogDetails);
			decision = { approved: true, state: "approved" };
		} else {
			this.logger.review("forwarded_permission.prompted", forwardedPermissionLogDetails);
			try {
				...
				decision = await this.requestPermissionDecisionFromUi(ctx.ui, "Permission Required (Subagent)", forwardedMessage);
			} catch (error) {
				logPermissionForwardingError(this.logger, "Failed to show forwarded permission confirmation dialog", error);
