...
import { getGlobalConfigPath, getProjectAgentsDir, getProjectConfigPath } from "../src/config-paths";
import { PermissionManager, type ScopedPermissionManager } from "../src/permission-manager";
import type { Rule, Ruleset } from "../src/rule";
import { createManager, createManagerWithProject } from "./helpers/manager-harness";
...
function checkTool(
	...
	return manager.check({ kind: "tool", surface: toolName, input, agentName }, sessionRules);
}
...
function checkPathValues(
	...
	return manager.check({ kind: "path-values", surface, values, agentName }, sessionRules);
}
...
describe("checkPermission — session rules", () => {
	it("session rule wins over the universal default (external_directory)", () => {
		...
		const result = checkTool(manager, "external_directory", { path: "/other/project" }, undefined, sessionRules);
	...
	it("session rule wins over the universal default (skill)", () => {
		...
		const result = checkTool(manager, "skill", { name: "librarian" }, undefined, sessionRules);
		expect(result.state).toBe("allow");
		expect(result.source).toBe("session");
	...
	it("session rule wins over the universal default (bash)", () => {
		...
		const result = checkTool(manager, "bash", { command: "git status" }, undefined, sessionRules);
		expect(result.state).toBe("allow");
		expect(result.source).toBe("session");
	...
	it("session rule with narrower pattern does not block a broader command not in session", () => {
		...
		const sessionRules: Ruleset = [sessionAllow("bash", "git status")];
		const result = checkTool(manager, "bash", { command: "git push origin main" }, undefined, sessionRules);
		expect(result.state).toBe("ask");
		expect(result.source).not.toBe("session");
	});
	...
	it("session wildcard pattern matches multiple commands", () => {
		...
		const push = checkTool(manager, "bash", { command: "git push origin main" }, undefined, sessionRules);
		const status = checkTool(manager, "bash", { command: "git status" }, undefined, sessionRules);
...
describe("PermissionManager with in-memory PolicyLoader", () => {
	...
	describe("session rule composition", () => {
		...
		it("session rule does not bleed across surfaces", () => {
			...
			const sessionRules: Ruleset = [sessionAllow("bash", "git *")];
			const bashResult = checkTool(manager, "bash", { command: "git status" }, undefined, sessionRules);
			expect(bashResult.state).toBe("allow");

			const readResult = checkTool(manager, "read", {}, undefined, sessionRules);
...
describe("checkPermission — per-tool path patterns", () => {
	...
	it("session rule for specific path overrides config deny", () => {
		...
		try {
			...
			const result = checkTool(manager, "read", { path: ".env" }, undefined, sessionRules);
			expect(result.state).toBe("allow");
			expect(result.source).toBe("session");
		} finally {
			cleanup();
...
describe("cross-cutting path surface", () => {
	...
	it("session approval on path surface overrides config deny", () => {
		...
		try {
			...
			const result = checkTool(manager, "path", { path: "/project/.env" }, undefined, sessionRules);
			expect(result.state).toBe("allow");
			expect(result.source).toBe("session");
		} finally {
			cleanup();
	...
	it(".env.example override: denies .env and .env.local, allows .env.example", () => {
		...
		try {
			...
			expect(checkTool(manager, "path", { path: ".env.local" }).state).toBe("deny");
			expect(checkTool(manager, "path", { path: ".env.production" }).state).toBe("deny");
			expect(checkTool(manager, "path", { path: "src/.env" }).state).toBe("deny");
			expect(checkTool(manager, "path", { path: ".env.example" }).state).toBe("allow");
			expect(checkTool(manager, "path", { path: "README.md" }).state).toBe("allow");
		} finally {
			cleanup();
		}
	});
...
describe("PermissionManager — configureForCwd and agentDir option", () => {
	...
	it("configureForCwd(cwd) applies project config (project overrides global)", () => {
		...
		try {
			...
			expect(checkTool(manager, "read", { path: "foo.txt" }).state).toBe("deny");

			manager.configureForCwd(cwd);

			// After configureForCwd: project override applies (last-match-wins)
			expect(checkTool(manager, "read", { path: "foo.txt" }).state).toBe("allow");
		} finally {
			cleanup();
		}
	});
	...
	it("configureForCwd(cwd) enforces permission: frontmatter from <cwd>/.pi/agents/<agent>.md (regression: #428)", () => {
		...
		try {
			...
			expect(checkTool(manager, "read", { path: "foo.txt" }, "coder").state).toBe("deny");
		} finally {
			cleanup();
		}
	});
...
`,
	...
	try {
		const allowed = checkTool(manager, "bash", { command: "git log --oneline" }, "reviewer");
		expect(allowed.state).toBe("allow");
		expect(allowed.matchedPattern).toBe("git log *");

		const denied = checkTool(manager, "bash", { command: "git status" }, "reviewer");
		expect(denied.state).toBe("deny");
...
`,
	...
	try {
		const reviewerResult = checkTool(manager, "custom_extension_tool", {}, "reviewer");
...
`,
	...
	try {
		const result = checkTool(manager, "mcp", { tool: "exa:web_search_exa" }, "reviewer");
		expect(result.state).toBe("allow");
		expect(result.source).toBe("mcp");
		expect(result.target).toBe("exa_web_search_exa");
	} finally {
...
`,
	...
	try {
		const result = checkTool(manager, "mcp", { tool: "web_search_exa" }, "reviewer");
		expect(result.state).toBe("deny");
		expect(result.source).toBe("mcp");
		expect(result.matchedPattern).toBe("exa_web_search_exa");
		expect(result.target).toBe("exa_web_search_exa");
...
`,
	...
	try {
		const allowed = checkTool(manager, "mcp", { tool: "web_search_exa" }, "reviewer");
		expect(allowed.state).toBe("allow");
		...
		const fallback = checkTool(manager, "mcp", { tool: "other_exa" }, "reviewer");
		expect(fallback.state).toBe("deny");
...
`,
	...
	try {
		...
		const mcpResult = checkTool(manager, "mcp", { tool: "exa:web_search_exa" }, "reviewer");
...
`,
	...
	try {
		const allowed = checkTool(manager, "skill", { name: "pi-code-review" }, "reviewer");
		expect(allowed.state).toBe("allow");
		expect(allowed.matchedPattern).toBe("pi-*");
		expect(allowed.source).toBe("skill");

		const asked = checkTool(manager, "skill", { name: "other-skill" }, "reviewer");
...
`,
	...
	try {
		const allowed = checkTool(manager, "external_directory", { path: `${homedir()}/Downloads/file.txt` }, "trusted");
		expect(allowed.state).toBe("allow");
		expect(allowed.matchedPattern).toBe("~/Downloads/*");
		expect(allowed.source).toBe("special");

		const denied = checkTool(manager, "external_directory", { path: `${homedir()}/Documents/secret.txt` }, "trusted");
		expect(denied.state).toBe("deny");
...
`,
	...
	try {
		const allowed = checkTool(manager, "skill", { name: "pi-code-review" }, "analyst");
		expect(allowed.state).toBe("allow");
		expect(allowed.matchedPattern).toBe("pi-*");

		const denied = checkTool(manager, "skill", { name: "other-skill" }, "analyst");
		expect(denied.state).toBe("deny");
		expect(denied.matchedPattern).toBe("*");
	} finally {
		cleanup();
...
test("checkPermission with empty session rules is identical to call without sessionRules arg", () => {
	...
	try {
		const withEmpty = checkTool(manager, "external_directory", { path: "/other/project/foo.ts" }, undefined, []);
...
test("session rules for one surface do not affect checks on other surfaces", () => {
	...
	try {
		...
		const mcpResult = checkTool(manager, "mcp", { tool: "exa:search" }, undefined, sessionRules);
...
test("checkPermission returns source 'session' for bash when session rules match", () => {
	...
	try {
		...
		const result = checkTool(manager, "bash", { command: "git status --short" }, undefined, sessionRules);
		expect(result.state).toBe("allow");
		expect(result.source).toBe("session");
		expect(result.matchedPattern).toBe("git *");
	} finally {
...
test("checkPermission returns source 'session' for bash when session rule is exact match", () => {
	...
	try {
		...
		const result = checkTool(manager, "bash", { command: "ls" }, undefined, sessionRules);
		expect(result.state).toBe("allow");
		expect(result.source).toBe("session");
	} finally {
		cleanup();
...
test("checkPermission falls back to config for bash when session rules do not match the command", () => {
	...
	try {
		...
		const result = checkTool(manager, "bash", { command: "npm run build" }, undefined, sessionRules);
		expect(result.state).toBe("deny");
		expect(result.source).toBe("bash");
	} finally {
		cleanup();
...
test("checkPermission returns source 'session' for mcp when session rules match the target", () => {
	...
	try {
		...
		const result = checkTool(manager, "mcp", { tool: "exa:search" }, undefined, sessionRules);
		expect(result.state).toBe("allow");
		expect(result.source).toBe("session");
	} finally {
		cleanup();
...
describe("checkPathPolicy", () => {
	...
	it("evaluates precomputed policy values against the path surface", () => {
		...
		try {
			const result = checkPathValues(manager, [`${cwd}/src/App.jsx`, "src/App.jsx"]);
			expect(result.state).toBe("allow");
			expect(result.matchedPattern).toBe(`${cwd}/*`);
			expect(result.source).toBe("special");
	...
	it("applies session rules over config", () => {
		...
		try {
			...
			const result = checkPathValues(manager, ["src/App.jsx"], undefined, sessionRules);
			expect(result.state).toBe("allow");
			expect(result.source).toBe("session");
		} finally {
			cleanup();
	...
	it("evaluates against the external_directory surface when one is provided", () => {
		...
		try {
			const result = checkPathValues(manager, ["/tmp/x"], undefined, undefined, "external_directory");
			expect(result.state).toBe("allow");
