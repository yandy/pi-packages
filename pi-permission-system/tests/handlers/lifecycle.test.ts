...
import { makeLogger, makeRealResolver, makeRealSession } from "../helpers/session-fixtures";
...
function makeSetup(opts?: { configIssues?: string[] }) {
	...
	const handler = new SessionLifecycleHandler(session, resolver, serviceLifecycle, logger, audit);
	return {
		handler,
		session,
		resolver,
