...
import { makeGateCheckResult as makeCheckResult, makeResolver } from "../../helpers/gate-fixtures";
...
describe("describePathGate", () => {
	...
	it("emits an access-path whose matchValues include the symlink-resolved form (#486)", () => {
		...
		realpathSync.mockImplementation((p: string) => (p === "/test/project/.env" ? "/vault/secret.env" : p));
		const resolver = makeResolver(makeCheckResult({ state: "allow" }));
