...
describe("buildDecisionEvent", () => {
	...
	it("normalises null agentName to null", () => {
		const event = buildDecisionEvent({ surface: "read", value: "read" }, makeCheck(), null, "deny", "policy_deny");
	...
	it("passes result and resolution through", () => {
		const event = buildDecisionEvent({ surface: "bash", value: "rm -rf /" }, makeCheck(), null, "deny", "user_denied");
