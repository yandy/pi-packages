...
function gateReturning(outcome: GateOutcome) {
	return vi.fn<(event: unknown, ctx: ExtensionContext) => Promise<GateOutcome>>().mockResolvedValue(outcome);
}
...
describe("createFailClosedToolCall", () => {
	it("translates an allow outcome to the empty SDK shape", async () => {
		...
		const boundary = createFailClosedToolCall(gateReturning({ action: "allow" }), reporter, audit, makeTracer());
	...
	it("writes a per-call decision trace with the tool name and action", async () => {
		...
		const boundary = createFailClosedToolCall(gateReturning({ action: "allow" }), makeReporter(), makeAudit(), tracer);
	...
	it("blocks fail-closed when the gate throws, recording an error and a review-log entry", async () => {
		...
		const boundary = createFailClosedToolCall(gate, reporter, audit, makeTracer());
