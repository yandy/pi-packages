...
function asChecker(manager: ScopedPermissionManager): SkillPermissionChecker {
	return {
		checkPermission: (surface, input, agentName) => manager.check({ kind: "tool", surface, input, agentName }),
	};
}
...
function makeManager(
	...
	return {
		checkPermission: vi.fn((_surface: string, input: unknown): PermissionCheckResult => {
...
test("REGRESSION: resolveSkillPromptEntries sanitizes every available_skills block", () => {
	...
	try {
		...
		const result = resolveSkillPromptEntries(prompt, asChecker(manager), null, "/cwd");
