...
describe("isPiInfrastructureRead", () => {
	...
	test("allows 'read' tool for a file inside agentDir", () => {
		expect(
			isPiInfrastructureRead("read", "/home/user/.pi/agent/extensions/pi-permission-system/config.json", INFRA_DIRS, CWD),
		).toBe(true);
	});
	...
	test("allows 'grep' tool for a path inside agentDir/git", () => {
		expect(isPiInfrastructureRead("grep", "/home/user/.pi/agent/git/some-package/README.md", INFRA_DIRS, CWD)).toBe(true);
	});
	...
	test("blocks 'write' tool for a file inside agentDir", () => {
		expect(
			isPiInfrastructureRead("write", "/home/user/.pi/agent/extensions/pi-permission-system/config.json", INFRA_DIRS, CWD),
		).toBe(false);
	});
	...
	test("allows 'read' for a path inside project-local .pi/npm/", () => {
		expect(isPiInfrastructureRead("read", `${CWD}/.pi/npm/node_modules/some-skill/SKILL.md`, INFRA_DIRS, CWD)).toBe(true);
	});
