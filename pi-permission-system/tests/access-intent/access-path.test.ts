...
describe("AccessPath.forPath", () => {
	...
	describe("matchValues()", () => {
		test("adds the symlink-resolved alias alongside the typed path", () => {
			...
			realpathSync.mockImplementation((p: string) => (p.startsWith("/tmp") ? `/private${p}` : p));
			expect(AccessPath.forPath("/tmp/x", { cwd }).matchValues()).toEqual(["/tmp/x", "/private/tmp/x"]);
		});

		test("deduplicates when the canonical form equals the lexical form", () => {
			expect(AccessPath.forPath("/etc/hosts", { cwd }).matchValues()).toEqual(["/etc/hosts"]);
		});
		...
		test("includes only the lexical aliases when canonical is empty", () => {
			...
			expect(AccessPath.forPath("/etc/hosts", { cwd }).matchValues()).not.toHaveLength(0);
		});
		...
		test("adds the canonical alias resolved against resolveBase", () => {
			realpathSync.mockImplementation((p: string) => (p === "/projects/my-app/sub/foo.ts" ? "/real/foo.ts" : p));
			expect(
				AccessPath.forPath("foo.ts", {
					cwd,
					resolveBase: "/projects/my-app/sub",
				}).matchValues(),
			).toEqual(["/projects/my-app/sub/foo.ts", "sub/foo.ts", "foo.ts", "/real/foo.ts"]);
		});
	});
	...
	describe("boundaryValue()", () => {
		test("returns the canonical (symlink-resolved) form", () => {
			realpathSync.mockImplementation((p: string) => (p.startsWith("/tmp") ? `/private${p}` : p));
			expect(AccessPath.forPath("/tmp/x", { cwd }).boundaryValue()).toBe("/private/tmp/x");
		});

		test("returns the lexical form when path has no symlinks", () => {
			expect(AccessPath.forPath("/etc/hosts", { cwd }).boundaryValue()).toBe("/etc/hosts");
		});
	...
	describe("value()", () => {
		...
		test("normalizes the path against cwd", () => {
			...
			expect(AccessPath.forPath("src/foo.ts", { cwd }).value()).toBe("/projects/my-app/src/foo.ts");
		});
